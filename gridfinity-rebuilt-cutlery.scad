...
module divider(thickness) {
    difference() {
        rotate([0,45,0])
        cube([28,thickness,28], center=true);

        union() {
            translate([0,0,-50/2])
            cube([50,thickness,50], center=true);

            translate([0,0,24/2+16])
            cube([50,thickness,24], center=true);
        }
    }
}
...
difference() {
    union() {
        ...
        num_of_dividers = 13;
        for (i = [0:num_of_dividers]) {
            translate([0,-40+(80/num_of_dividers)*i,10.5])
            divider(2);
        }
    }
