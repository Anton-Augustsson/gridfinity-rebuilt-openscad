...
num_blades_stack = 6;
start_hight = 3.8+1.81;
union() {
    translate([8,-9.6-3,start_hight])
    for ( i = [0:1.2:1.2*num_blades_stack])
    translate([0,0,i])
    polyhedron( TrapezoidPoints, TrapezoidFace );

    translate([-60.2-8,-9.6-3,start_hight])
    for ( i = [0:1.2:1.2*num_blades_stack])
    translate([0,0,i])
    polyhedron( TrapezoidPoints, TrapezoidFace );

    translate([-38,10,start_hight-0.02+15/2])
